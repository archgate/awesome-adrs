...
Desktop applications often need to import data from external services such as APIs, file exports, or third-party platforms. These data import jobs have unique requirements in a desktop context: they must run as separate processes to avoid blocking the main application, they must be safe to retry without creating duplicate data, they must be configurable through both CLI arguments and environment variables, and they must be distributable alongside the desktop application without requiring users to install a Python runtime.

Running import jobs as sidecar processes that are packaged as standalone executables solves all of these concerns. The desktop application spawns the sidecar when the user triggers an import, monitors its progress, and handles the result. Each job is idempotent so that if a job fails partway through, the user can simply retry it without worrying about duplicate records.
...
- **Event-driven triggering:** Jobs are triggered by user actions in the desktop application (e.g., clicking an import button), not by fixed schedules or cron jobs.
...
- Single-file executables simplify desktop application distribution; no Python runtime required on user machines.
