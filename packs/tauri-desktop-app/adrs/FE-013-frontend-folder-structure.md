...
For desktop applications built with Tauri, predictable structure is especially important: it reduces onboarding time, enables AI/LLM agents to navigate the codebase effectively, supports parallel development, and clearly separates routes, pages, and reusable components.

## Decision

The frontend must follow this standardized folder structure within `packages/frontend/src/`:

- **`routes/`** -- TanStack Router file-based route definitions. Contains route files like `__root.tsx`, `index.tsx`, and nested route directories. These files define the routing tree and reference page components.
...
- **`queries/`** -- TanStack Query hooks, query key factories, and mutations organized by domain (e.g., `queries/users.ts`).
- **`atoms/`** -- Jotai atoms for client-side global state (e.g., `atoms/ui.ts`, `atoms/preferences.ts`).
- **`api/`** -- API client setup such as the Hono RPC client configuration (e.g., `api/client.ts`).
...
- Edit `routeTree.gen.ts` manually; it is auto-generated by TanStack Router.
- Mix page components with reusable components in the same directory.
- Use `index.ts` files for re-exporting modules (see the No Barrel Files ADR).
